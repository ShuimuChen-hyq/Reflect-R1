...
class VideoFrameSeeker(BaseTool):
    ...
    def call(self, params: Union[str, dict], multimodal_cache: Dict, **kwargs):
        params = self.cast(params) # {'query': 'Colt Model 1855 Revolving Carbine', 'start_time': 0.0, 'end_time': 1623.21, 'num_frames': 8} ## 简单的数据清洗
        params = self._verify_json_format_args(params) # json格式检查
    ...
    def seek_video_frames(self, query: str, start_time: int, end_time: int, num_frames: int, multimodal_cache: Dict) -> List[Dict[str, int]]:
        ...
        if FORCE_NUM_KEY_FRAMES > 0: # 强制使用预设的帧数 不执行
        ...
        response_content = construct_temporal_augmented_frames(timestamps, selected_frames) # 在每一帧之前加入时间标识
...
#     {"type": "text", "text": "2.1s"},
#     # 第1帧图像数据
#     {"type": "image", "image": torch.Tensor([[...]])},  # 形状例如 [3, 224, 224]
    
#     # 第2个时间戳
#     {"type": "text", "text": "5.0s"},
#     # 第2帧图像数据
#     {"type": "image", "image": torch.Tensor([[...]])},
